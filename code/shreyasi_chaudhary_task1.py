...
data = sc.textFile(input_file,numOfPartitions)
...
baskets = data.groupByKey().map(lambda x: set(x[1]))
...
#PHASE 2 started
map_phase2 = baskets.mapPartitions(countSubsets).filter(lambda x: x[1] >= support_partitions).reduceByKey(lambda x, y: x+y).filter(lambda x: x[1] >= support).map(lambda x: x[0])
...
#../spark-2.3.2-bin-hadoop2.7/bin/spark-submit shreyasi_chaudhary_task1.py 1 4 small2.csv res_1_final.txt
#../spark-2.3.2-bin-hadoop2.7/bin/spark-submit shreyasi_chaudhary_task1.py 2 9 small2.csv res_2_final.txt